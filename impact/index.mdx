---
title: Impact & Outreach
description: >-
  Discover how Bactopia is making an impact and giving back to the community.
sidebar_position: 1
---

import {CardGrid, Card} from '@site/src/components/CardGrid';

# Impact & Outreach

Bactopia's 1 million-plus downloads attest to its reliability and widespread use. Below are
a few links to learn more about Bactopia's impact and outreach to the community.

<CardGrid columns={2}>
  <Card
    icon="material-heart"
    title="Kudos to a selfless community"
    description="See the full set of community resources used by Bactopia and how to cite them"
    link="/impact-and-outreach/acknowledgements"
    linkText="Acknowledgements"
  />
  <Card
    icon="simple-googlescholar"
    title="Empowering scientific research"
    description="Get to know the role Bactopia is playing to support academic studies"
    link="/impact-and-outreach/citations"
    linkText="Citations"
  />
  <Card
    icon="material-tools"
    title="Supporting developers"
    description="Discover how Bactopia is regularly contributing back to the community"
    link="/impact-and-outreach/enhancements"
    linkText="Enhancements to OSS"
  />
  <Card
    icon="octicons-video-24"
    title="Explore past content"
    description="Learn more about Bactopia from selected talks, posters, and blog posts"
    link="/impact-and-outreach/presentations"
    linkText="Presentations"
  />
</CardGrid>

## Funding

Support for this project came (in part) from an Emory Public Health Bioinformatics Fellowship
funded by the [CDC Emerging Infections Program (U50CK000485) PPHF/ACA: Enhancing Epidemiology and Laboratory Capacity](https://dph.georgia.gov/EIP),
the [Wyoming Public Health Division](https://health.wyo.gov/publichealth/), and
the [Center for Applied Pathogen Epidemiology and Outbreak Control (CAPE)](https://www.linkedin.com/company/center-for-applied-pathogen-epidemiology-and-outbreak-control/).

<div style={{display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1rem', flexWrap: 'wrap', margin: '2rem 0'}}>
  <a href="https://dph.georgia.gov/EIP">
    <img src="/img/gaeip-banner.png" alt="Georgia Emerging Infections Program" style={{width: '140px', height: 'auto', borderRadius: '0.25rem'}} />
  </a>
  <a href="https://health.wyo.gov/publichealth/">
    <img src="/img/wyphd-banner.jpg" alt="Wyoming Public Health Division" style={{width: '280px', height: 'auto', borderRadius: '0.25rem'}} />
  </a>
  <a href="https://www.linkedin.com/company/center-for-applied-pathogen-epidemiology-and-outbreak-control/">
    <img src="/img/cape-banner.png" alt="Center for Applied Pathogen Epidemiology and Outbreak Control" style={{width: '185px', height: 'auto', borderRadius: '0.25rem'}} />
  </a>
</div>
